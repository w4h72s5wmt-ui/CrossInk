...
class NotesFastKeyboardActivity : public Activity {
 ...
 private:
  ...
  // Notes-only typing fast path. The panel still uses the stock full-screen
  // FAST waveform from the validated firmware; we reduce how often it runs and
  // avoid rebuilding the static keyboard bitmap on every character.
  uint8_t* keyboardCache = nullptr;
  size_t keyboardCacheSize = 0;
