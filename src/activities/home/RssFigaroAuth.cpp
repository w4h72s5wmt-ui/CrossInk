...
struct ParsedUrl { bool https = false; std::string host; std::string path; uint16_t port = 80; };
struct Session {
...
bool parseUrl(const std::string& url, ParsedUrl& out) {
  ...
  if (portSep == std::string::npos) out.host = hostPort;
  else {
...
bool isAllowedUrl(const std::string& url) {
  ParsedUrl p;
  return parseUrl(url, p) && p.https && p.port == 443 && isFigaroHost(p.host);
}
...
bool isRedirect(const int status) { return status == 301 || status == 302 || status == 303 || status == 307 || status == 308; }

char combinedByteAt(const uint8_t* data, const size_t index) {
  return index < session.tailLength ? session.tail[index] : static_cast<char>(data[index - session.tailLength]);
}
size_t findMarker(const uint8_t* data, const size_t length, const char* marker) {
  const size_t markerLength = std::strlen(marker), combinedLength = session.tailLength + length;
  if (!data || !length || markerLength > combinedLength) return std::string::npos;
  for (size_t i = 0; i + markerLength <= combinedLength; ++i) {
    size_t j = 0;
    while (j < markerLength && std::tolower(static_cast<unsigned char>(combinedByteAt(data, i + j))) ==
                               std::tolower(static_cast<unsigned char>(marker[j]))) ++j;
    if (j == markerLength) return i + markerLength <= session.tailLength ? 0 : i + markerLength - session.tailLength;
  }
  return std::string::npos;
}
void rememberTail(const uint8_t* data, const size_t length) {
  const size_t keep = sizeof(session.tail);
  if (length >= keep) { session.tailLength = keep; std::memcpy(session.tail, data + length - keep, keep); return; }
  const size_t oldKeep = std::min(session.tailLength, keep - length);
  if (oldKeep) std::memmove(session.tail, session.tail + session.tailLength - oldKeep, oldKeep);
...
void resetRequestState(const HttpDownloader::DataCallback& onData, const HttpDownloader::CancelCallback& shouldCancel,
  ...
  session.cancelled = session.receiverRejected = session.encodedResponse = session.encodedRejected = false;
  session.seenArticle = session.articleClosed = false;
  session.bytesReceived = session.tailLength = 0;
}
...
esp_err_t onHttpEvent(esp_http_client_event_t* event) {
  ...
  if (event->event_id == HTTP_EVENT_ON_HEADER && event->header_key && event->header_value) {
    if (session.diagnostic && session.diagnostic->headersMs == 0)
      session.diagnostic->headersMs = static_cast<uint32_t>(millis() - session.diagnostic->startedMs);
    if (strcasecmp(event->header_key, "Location") == 0) session.redirectLocation.assign(event->header_value);
    if (strcasecmp(event->header_key, "Content-Encoding") == 0 && strcasecmp(event->header_value, "identity") != 0)
      session.encodedResponse = true;
    return ESP_OK;
  }
  ...
  if (session.diagnostic) session.diagnostic->received += length;
  ...
  if (!session.onData || !*session.onData || !(*session.onData)(data, length)) {
    session.receiverRejected = true;
    return ESP_FAIL;
  }
  if (session.diagnostic) session.diagnostic->stored += length;
  if (!session.seenArticle && findMarker(data, length, ARTICLE_START) != std::string::npos) session.seenArticle = true;
  if (session.seenArticle && findMarker(data, length, ARTICLE_END) != std::string::npos) {
    session.articleClosed = true;
    if (session.diagnostic && !session.diagnostic->articleClosed) {
      session.diagnostic->articleClosed = true;
      session.diagnostic->articleMs = static_cast<uint32_t>(millis() - session.diagnostic->startedMs);
    }
  }
...
bool isConfiguredFor(const std::string& url) { return isAllowedUrl(url) && (ensureCookie() || Storage.exists(AUTH_PATH)); }
...
HttpDownloader::DownloadError streamUrl(const std::string& url, const HttpDownloader::DataCallback& onData,
  ...
  if (!onData || !isAllowedUrl(url) || !ensureCookie()) return fail(HttpDownloader::HTTP_ERROR);
  ...
  for (uint8_t hop = 0; hop < MAX_REDIRECTS; ++hop) {
    ...
    esp_err_t setup = esp_http_client_set_timeout_ms(session.client, HTTP_TIMEOUT_MS);
    if (setup == ESP_OK) setup = esp_http_client_set_header(session.client, "Cookie", session.cookie.c_str());
    if (setup != ESP_OK) { diagnostic.detail = setup; return fail(HttpDownloader::HTTP_ERROR); }

    diagnostic.phase = "perform";
    const uint32_t startMs = millis();
    const esp_err_t result = esp_http_client_perform(session.client);
    diagnostic.bodyMs = static_cast<uint32_t>(millis() - startMs);
    diagnostic.detail = result;
    const int status = esp_http_client_get_status_code(session.client);
    diagnostic.httpStatus = status;
    diagnostic.expected = esp_http_client_get_content_length(session.client);
    if (session.cancelled) return finish(HttpDownloader::ABORTED, true);
    ...
    if (isRedirect(status)) {
      ...
      const std::string location = session.redirectLocation;
      const std::string nextUrl = buildRedirectUrl(currentUrl, location);
      clearRequestState();
      dropClient();
      if (location.empty() || !isAllowedUrl(nextUrl)) return fail(HttpDownloader::HTTP_ERROR);
      currentUrl = nextUrl;
      continue;
    }
    if (status != 200) { diagnostic.phase = "http-status"; return fail(HttpDownloader::HTTP_ERROR); }
    if (!session.articleClosed) {
      diagnostic.phase = result == ESP_OK ? "missing-article-end" : "perform-error";
      return fail(HttpDownloader::HTTP_ERROR);
    }
    diagnostic.phase = result == ESP_OK ? "response-end" : "tail-after-article";
    return finish(HttpDownloader::OK, result != ESP_OK || !esp_http_client_is_persistent_connection(session.client));
  }
