...
void NotesFastKeyboardActivity::releaseKeyboardCache() {
  if (keyboardCache != nullptr) {
    heap_caps_free(keyboardCache);
    keyboardCache = nullptr;
  }
  keyboardCacheSize = 0;
  keyboardCacheKey = 0;
  keyboardCacheValid = false;
...
uint32_t NotesFastKeyboardActivity::keyboardVisualKey() const {
  ...
  key = key * 2u + (showLangKey ? 1u : 0u);
  key = key * 2u + (shifted ? 1u : 0u);
  key = key * 2u + (symbols ? 1u : 0u);
  key = key * 2u + (urlPanel ? 1u : 0u);
  key = key * 2u + (cursorMode ? 1u : 0u);
  key = key * 2u + (touchSelectionHidden ? 1u : 0u);
  if (!cursorMode && !touchSelectionHidden) {
    key = key * 64u + static_cast<uint32_t>(selectedLogicalIndex() + 1);
  }
  return key;
}
...
void NotesFastKeyboardActivity::render(RenderLock&&) {
    ...
    textWidth = measureRange(displayText, lineStartIdx, lineEndIdx);
    {
      ...
      if (!cursorDrawn && cursorPos >= lineStartIdx &&
        ...
        if (isPassword && !passwordVisible && cursorMode) {
          ...
          beforeWidth =
              renderer.getTextAdvanceX(UI_12_FONT_ID, maskedBeforeCursor.c_str(), EpdFontFamily::REGULAR);
        } else {
        ...
        if (cursorCharBytes > 0) {
          if (isPassword && !passwordVisible && cursorMode) {
            std::string beforeAndCursor = maskedBeforeCursor;
            beforeAndCursor += displayCursorChar;
            throughCursorWidth =
                renderer.getTextAdvanceX(UI_12_FONT_ID, beforeAndCursor.c_str(), EpdFontFamily::REGULAR);
          } else {
            throughCursorWidth =
                measureRange(displayText, lineStartIdx, static_cast<int>(cursorPos + cursorCharBytes));
          }
  ...
  if (!restoredKeyboard) {
    ...
    props.bottomHitOverflow =
        static_cast<int16_t>(std::max(0, hintsTop - (kbRect.y + kbRect.height)));
