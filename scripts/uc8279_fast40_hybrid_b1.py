...
#   * Test ONLY active table 0x22 with the same 4->1 cut.
#   * 0x23 remains Fast40 as the protected active transition.
# Safety boundary unchanged: no analog rails, PLL, TSSET/CDI/PFS/GATE_SCAN/CCSET changes.

helper = r'''
void buildFast40HybridB1PreBw(uint8_t out[5][PREBW_LUT_LEN]) {
  ...
  // Hybrid-A tables 0x21/0x24 plus one isolated active transition 0x22.
...
new_setup = r'''  uint8_t hybridLuts[5][PREBW_LUT_LEN];
  buildFast40HybridB1PreBw(hybridLuts);
  ...
  powerOnIfNeeded(bus, " 8279x4_win_fast40_hybridB1_PON");
  bus.cmd(CMD_DISPLAY_REFRESH);
'''
...
replace_once(
    ...
    '  bus.waitRefreshComplete(" 8279x4_window_fast40_hybridB1_DRF");\n\n'
    '  // Keep DTM1 synchronized only inside the region that actually changed. Pixels\n',
)

bench = r'''void MissileCommandActivity::drawPlayingFrame() {
  {
    constexpr const char* BENCH_PATH = "/missile-command-fast40-hybrid-b1-bench.csv";
    ...
    if (!benchFilePrimed) {
      FsFile file;
      if (Storage.openFileForWrite("MISSILE-HYBRID-B1", BENCH_PATH, file)) {
        ...
        const char started[] = "fast40-hybrid-b1-started,0,0,0,0,0\n";
      ...
      if (raw > 0 && raw < 2000000) {
        ...
        if (benchSamples >= BENCH_TARGET_SAMPLES) {
          ...
          if (Storage.openFileForWrite("MISSILE-HYBRID-B1", BENCH_PATH, file)) {
            ...
            file.write(reinterpret_cast<const uint8_t*>(header), sizeof(header) - 1);
            char line[128];
            const int n = std::snprintf(line, sizeof(line), "fast40-hybrid-b1,%lu,%lu,%lu,%lu,%lu\n",
...
print("UC8279 Fast40 Hybrid-B1 (0x22 isolated) + 200-frame gameplay benchmark applied")
