...
replace_once(CPP, "constexpr int64_t GAME_FRAME_US = 100000;\n",
             "constexpr int64_t GAME_FRAME_US = 100000;\n"
             "// Physical 1:1 gameplay viewport. This is NOT upscaled: limiting panel PTL\n"
             "// to 600x320 keeps the dynamic area at 50% of the 800x480 panel.\n"
             "constexpr int GAME_VIEW_W = 600;\n"
             "constexpr int GAME_VIEW_H = 320;\n"
             "constexpr int GAME_EFFECT_MARGIN = 40;\n")
...
new_geometry = '''GameGeometry gameGeometry(const GfxRenderer& renderer, const MappedInputManager& mappedInput) {
  ...
  const int statusH = std::max(36, renderer.getLineHeight(UI_10_FONT_ID) + 10);
  constexpr int gap = 4;
  const Rect status{viewX, viewY, viewW, statusH};
  const Rect field{viewX, viewY + statusH + gap, viewW, std::max(100, viewH - statusH - gap)};
...
replace_once(CPP, "  sceneNeedsFullRedraw_ = true;\n  cycleRenderPending_.store(false);\n",
...
print("Direct 600x320 Missile app migration complete")
