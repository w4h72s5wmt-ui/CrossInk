...
# Fast40 Hybrid-A:
#   * Start from the visually-clean Fast40 quantized waveform.
#   * Preserve the polarity-changing tables (0x22/0x23) exactly at Fast40.
#   * Only shorten 4-frame phases to 1 frame in the same-state tables
#     0x21 and 0x24 (conventional WW/BB ordering).
#   * 0x20 VCOM remains Fast40.
# This deliberately avoids the global 4->1 jump that made Fast35 artifact.
#
# Safety boundary unchanged: no PWR/VDCS/BTST, no analog voltage, no PLL,
# no TSSET/CDI/PFS/GATE_SCAN/CCSET changes.
...
bench = r'''void MissileCommandActivity::drawPlayingFrame() {
  {
    ...
    if (!benchFilePrimed) {
      FsFile file;
      if (Storage.openFileForWrite("MISSILE-HYBRID-A", BENCH_PATH, file)) {
      ...
      if (raw > 0 && raw < 2000000) {
        ...
        if (benchSamples >= BENCH_TARGET_SAMPLES) {
          ...
          if (Storage.openFileForWrite("MISSILE-HYBRID-A", BENCH_PATH, file)) {
            ...
            file.write(reinterpret_cast<const uint8_t*>(header), sizeof(header) - 1);
            char line[128];
            const int n = std::snprintf(line, sizeof(line), "fast40-hybrid-a,%lu,%lu,%lu,%lu,%lu\n",
...
print("UC8279 Fast40 Hybrid-A + 200-frame gameplay benchmark applied")
