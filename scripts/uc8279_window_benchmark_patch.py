...
# Temporary experimental bridge: expose the SDK window primitive through HalDisplay.
replace_once(
    "lib/hal/HalDisplay.h",
    "  void displayBuffer(RefreshMode mode = RefreshMode::FAST_REFRESH, bool turnOffScreen = false);\n",
    "  void displayBuffer(RefreshMode mode = RefreshMode::FAST_REFRESH, bool turnOffScreen = false);\n"
    "  // Experimental UC8279 benchmark path: physical panel coordinates, byte-aligned X.\n"
...
# Add a genuine UC8279 X4 window override. Stage A intentionally keeps full DTM
# uploads so the benchmark isolates PTL/waveform scan time from SPI transfer time.
replace_once(
...
void Uc8279X4Driver::displayWindow(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, uint16_t x, uint16_t y,
  ...
  // A partial window needs a known DTM1 baseline. The first request after boot
  // therefore uses the driver's ordinary path, which performs its normal clean
  // initialization and baseline seeding.
  if (_needFullClear || !_oldPlaneValid || _redriveAfterGray) {
    display(bus, fb, prev, RefreshMode::Fast, turnOff);
    return;
  }

  // Stage A benchmark deliberately leaves transfer geometry untouched: DTM2 is
  // still uploaded as a complete 800x600-addressed plane, while PTL alone is
  // reduced. This tells us whether the UC8279 waveform scan itself scales with
  // the window before we optimize RAM traffic.
  ...
  // Keep the controller's OLD plane authoritative for the next window. Stage A
  // intentionally does this as a full-plane upload too; it is not part of the
  // measured DRF waveform ceiling we are trying to identify.
...
# App-local benchmark. It runs once, immediately after the first synchronized
# gameplay frame has completed, so the controller baseline is known. Only a tiny
# 16x16 marker is toggled while the requested PTL grows; each size is run twice,
# returning the framebuffer and panel to the exact original content.
replace_once(
...
replace_once(
    ...
    "constexpr const char SCORE_PATH[] = \"/.crosspoint/missile-command-score.bin\";\n"
    "constexpr const char WINDOW_BENCH_PATH[] = \"/missile-command-window-benchmark.csv\";\n",
)

bench_fn = r'''
void runUc8279WindowBenchmark() {
  FsFile file;
  if (!Storage.openFileForWrite("MISSILE-WINDOW", WINDOW_BENCH_PATH, file)) return;
...
print("UC8279 window benchmark patch applied")
