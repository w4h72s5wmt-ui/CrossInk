...
# Real-gameplay Fast35 experiment.
# Same stock X4 Pro transition LUT, same rail/polarity codes and phase order.
# Only lower-six-bit phase frame durations are scaled from 100% to 35%.
# The gameplay benchmark creates its CSV immediately, then writes the final
# cadence after 200 real synchronized gameplay frames.
#
...
helper = r'''
void buildFast35PreBw(uint8_t out[5][PREBW_LUT_LEN]) {
  constexpr uint8_t speedPct = 35;
...
'''
new_setup = r'''  // Stage Fast35 real-gameplay path.
  ...
  uint8_t fast35Luts[5][PREBW_LUT_LEN];
  buildFast35PreBw(fast35Luts);
  ...
  for (uint8_t t = 0; t < 5; ++t) {
    ...
    bus.data(fast35Luts[t], PREBW_LUT_LEN);
  }

  powerOnIfNeeded(bus, " 8279x4_win_fast35_PON");
  bus.cmd(CMD_DISPLAY_REFRESH);
'''
...
replace_once(
    ...
    '  bus.waitRefreshComplete(" 8279x4_window_fast35_DRF");\n\n'
    '  // Keep DTM1 synchronized only inside the region that actually changed. Pixels\n',
)

bench = r'''void MissileCommandActivity::drawPlayingFrame() {
  {
    constexpr const char* BENCH_PATH = "/missile-command-fast35-gameplay-bench.csv";
    ...
    if (!benchFilePrimed) {
      FsFile file;
      if (Storage.openFileForWrite("MISSILE-FAST35-BENCH", BENCH_PATH, file)) {
        ...
        const char started[] = "fast35-started,0,0,0,0,0\n";
...
print("UC8279 Fast35 gameplay + immediate 200-frame cadence benchmark applied")
